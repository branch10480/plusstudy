	...
	<div id="headerContent" class="plot cf">
		...
		<nav id="hNav">
			<ul class="cf">
				...
				<li class="hNavst2"><?php echo $this->Html->image(IMG_PATH . 'myprofile_btn.png', array("alt" => "マイプロフィール", 'url' => array('controller' => 'Accounts', 'action' => 'profile','?' => array('id' => $this->Session->read('Auth.id'))))); ?></li>
				<li class="hNavst2"><?php echo $this->Html->image(IMG_PATH . 'logout_btn.png', array("alt" => "ログアウト", 'url' => array('controller' => 'TeachMes', 'action' => 'index'))); ?></li>
			</ul>
		</nav>
	</div>
</header>
