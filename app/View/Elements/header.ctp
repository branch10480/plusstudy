	...
	<div id="headerContent" class="plot cf">
		...
		<nav id="hNav">
			<ul class="cf">
				...
				<li class="hNavMyprof"><?php echo $this->Html->image(IMG_PATH . $this->Session->check('Auth.profImg') ? $this->Session->read('Auth.profImg') : NO_IMG_URL, array("alt" => "マイプロフィール", 'url' => array('controller' => 'Accounts', 'action' => 'profile','?' => array('id' => $this->Session->read('Auth.id'))))); ?></li>
